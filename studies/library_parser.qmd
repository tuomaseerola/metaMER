...
model_result_2_df <- function(x) {
  ...
  model_attributes <- str_split_1(model_attributes, '\\.')
  names(model_attributes) <- c('feature_id', 'data_id', 'experiment_id')
  # split across '_' to 
  model_measures <- data.frame(do.call('rbind', str_split(colnames(x), '_')))
  print(model_measures)
  names(model_measures) <- c('dimension', 'measure')
  model_measures_split <- str_split(model_measures[,'measure'], '\\.')
...
```         
library = rbind(features.data.exp = c(dim_measure.summary = 0.5, dim_measure.summary = 0.1), features.data.exp = c(dim_measure.summary = 0.2, dim_measure.summary = 0.5))

# e.g.

librosa = rbind(mfcc.msd.1 = c(valence_r2.mean = 0.5, arousal_r2.mean = 0.1), 
...
this_study <- data.frame(citekey = c('testStudy2023ab','testStudy2023ab', 'testStudy2020of'),
  model_rate_emotion_values = c("marsyas = rbind(mfcc.msd.1 = c(valence_r2.mean = 0.9, arousal_r2.mean = 0.5,
                                  valence_r2.sd = 0.4, arousal_r2.sd = 0.3,
  valence_mse.mean = 0.3, arousal_mse.mean = 9.1))",
  "librosa = rbind(mfcc.ismir.1 = c(valence_r2.mean = 0.1, arousal_r2.mean = 0.3,
                                  valence_r2.sd = 0.2, arousal_r2.sd = 0.4,
  euclid_euclid.mean = 0.32, euclid_euclid.sd = 0.1))",
  "mirtoolbox = rbind(pitch.filmMusic.2 = c(valence_r2.mean = 0.1, arousal_r2.mean = 0.3))")
)
```
