...
Moving on the arousal, we carry out the same meta-analytical analysis applying the random-effects model to arousal. Table 3 and Figure 3 show the same outputs as was done for valence describing the broad pattern of results in tabular format, and visually illustrate the spread and heterogeneity of all studies. 

Table 3 indicates that the overall correlation across the studies using the best performing model out of each study (Max) is 0.807 (CI95% 0.745-0.855). If we examine all the models, the correlation drops to 0.796, only marginally lower.


The indicators of heterogeneity ($\tau^2$ = 0.155 and $I^2$=96.8%) suggest high 
