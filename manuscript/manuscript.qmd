...
Emotional engagement is one of the central reasons why people engage with music in their every day activities and why music is being implicated in various health applications [@juslin2022emotions;@agres2021music]. Significant advances in music information retrieval relating to music emotion recognition (MER) tasks have been made in recent years [@gomez2021;@panda2020audio]. The available features, modelling techniques and datasets have given the field opportunities to refine the accuracy and reliability of predicting annotated emotions from audio. Numerous studies over the last 25 years have established what emotions listeners perceive and recognise in music. In the last 15 years, it has become possible to trace the recognised emotions to musical contents such as expressive features [@lindstrom2003expressivity], structural aspects of music [@eerola_friberg_bresin_2013; @anderson2022ex; @grimaud_eerola_2022], or acoustic features [@yang2008; @panda2013multi; @saari_et_al_2015; @eerola2011c; @panda2020audio] or emergent properties identified through deep learning [@er2019music; @sarkar2020recognition].
...
Our aim is to establish the level of predictive accuracy for both models of emotional expression that can account for track-specific coordinates in (a) affective circumple space (valence and arousal) and (b) classification of emotion categories based on available and recent studies. We seek to identify the types of issues (modelling techniques, features, and musical qualities used) that significantly influence the prediction rates. To achieve these aims, we carry out a meta-analysis focused on journal articles published in the last 10 years. Based on literature, we articulate broad hypotheses such as arousal being predicted to a higher degree than valence, which is more challenging and more context dependent than arousal. For classification, simple utilitarian emotions (e.g., fear, anger) will be easier to predict than complex social emotions (e.g., sadness, nostalgia).
...
The results indicate the valence can be accounted generally moderately well, _r_ = 0.658 (95%CI 0.557-0.740) for valence Max (the best model from each of the 24 studies), while all models (n=120) encoded in these studies yield significantly lower overall prediction rate ( _r_ = 0.567). We argue that the lower correlation is due to inclusion of baselines these studies report and therefore not a true measure of the success in the task for our analysis. Further on, looking at the between-study heterogeneity as indexed by $\tau^2$ (0.138) and Higgins & Thompson's $I^2$ statistic [@higgins2002quantifying] for 98.2%, which captures the percentage of variability in the effect size, both indicate substantial heterogeneity. Since $I^2$ is heavily influenced by the size of the studies (large N having low sampling error), the index is perhaps not so insightful here whereas $\tau^2$ less insentive to the size and the number of studies and directly related to the outcome variable (_r_) size, we draw our interpretations about heteregeneity from the latter. To understand the observed effects across the studies and the type of heterogeneity evident in the data, we provide Figure 2 showing a forest plot of the random effects model across all studies using the best model. [TO BE CONTINUED]
