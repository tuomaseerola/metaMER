...
- Following one decade of competitions, the AMC task's highest performing model reached 69.83% accuracy [@park2017representation]. In parallel, other researchers began assessing MER as a regression task. The first study to popularize this approach predicted valence (i.e., the negative---positive emotional quality) and arousal (i.e., the calm---exciting quality) in 195 Chinese pop songs [@yang2008] using audio-extracted features. The regression model predicted arousal with 62% accuracy, but only achieved 28% accuracy for valence. This difference in prediction accuracy between dimensions has reappeared in several subsequent studies [e.g., @bai2016dimensional; @coutinho2013psychoacoustic], with some research suggesting this challenge reflects fewer well-established predictors and more individual differences for valence than arousal [@yang2007music; @eerola2011c].

<!--paragraph on models used across regession & classification tasks-->

Since the early days of MER, researchers have adopted a wide range of models to predict emotions, ranging in complexity from linear regression models [@saizclar2022pr; @griffiths2021am; @yang2008] to neural networks [@orjesek2022en; @hizlisoy2021mu]. Whereas early classification studies commonly employed Gaussian mixture models [@lu_automatic_2005; @liu_automatic_2003] and support vector machines [@mandel2007labrosa; @lin2009eeg; @tzanetakis2007marsyas], convolutional, recurrent, and fully-connected neural networks have become increasingly popular in recent years [@grekow2021music; @song2018audio; @coutinho2017sh]. In regression tasks, a wide range of algorithms have been tested, including partial least squares [@gingras2014be; @wang2021ac], support vector machines [@grekow2018au; @hu2017cr; @agarwal2021an], random forests [@beveridge2018po; @xu2021us], and convolutional neural networks [@orjesek2022en; @orjesek2022en].

## Large vs. small data

Researchers have assessed regression and classification techniques on diverse datasets with features drawn from music (e.g., audio, MIDI, metadata) and participants (e.g., demographic information, survey responses, physiological signals, etc.). To facilitate model comparison, several databases have been shared publicly, including MediaEval [@soleymani20131000], DEAM[@aljanaki2017developing], and AMG1608[@chen2015amg1608]. These datasets, represent Western pop music, are moderate in terms of the size (containing from 744 to 1802 music excerpts) and have been manually annotated by a relatively large number of participants (either by experts, students, or crowdsourced workers). Many datasets include audio features analyzed with software toolboxes for audio analysis such as OpenSMILE [@] and MIRToolbox [@lartillot2007matlab]---enabling models that predict ratings using tens, or even hundreds, of audio features. 

Although MER studies increasingly perform prediction on large-scale datasets, others use small datasets to evaluate models trained for specific contexts. For example, @gingras2014be evaluates the effect of amplitude normalization on emotion ratings using 84 six-second classical excerpts rated by two separate groups of 30 participants. @griffiths2021am test MER in a multi-genre context using two corpora with 20 and 40 songs across 20 distinct genres. @saizclar2022pr predicts the affective dimensions of the circumplex model in 40 short (8-12s) piano excerpts using onset curves (changes in the energy of a note starting from its onset). 

<!--MER approaches: psychology, engineering approaches-->
```{=html}
<!-- 
- Big picture: explanation vs predicting
- Summary of findings, success with MER approaches.
  - Scope of topic areas explored
  - Annotations: sometimes presented as a game, either experts or participants,
  sometimes secondary data from web databases (LastFM). Treated as "ground truth" (reference standard may be more accurate). Training/testing/validation. Comparisons with previously collected data/benchmarks
  - Participants: e.g., 5 experts, MTurk, etc.
  - Demographics
  - Common stimuli/genres: predominately pop
- Summary of findings, success with psychological approaches
  - Scope of topic areas explored
  - Annotations: Used to explain variance in participants' responses. Cross-validation rare.
  - Participants: psychology students, Prolific/MTurk?
  - Demographics
  - Common stimuli/genres: predominately classical
-->
```

## The semantic gap in MER / What are the reported standards? / ceiling
```{=html}
<!--
I'm considering now reworking this paragraph and de-emphasizing the semantic gap given the limited extent to which can address this. It might be worth including to show the semantic gap has narrowed, and we can point to the Barthet estimate, or to conclude that "linear methods with small to mid-sized feature sets appear perform best for this dimension".

- semantic challenge
- inherent error in all stages (annotation, features, models)

-->
```

The difficulty in predicting valence reflects a broader challenge in information retrieval. Specifically, relations between low-level predictors from music and text and the perceptual phenomena they model remain poorly understood, reaching a ceiling in prediction accuracy [@celma_foafing_2006]. To address this so-called *semantic gap*, researchers have attempted to identify new feature sets with greater relevance to emotion [@panda2020audio; @chowdhury2021perceived], combine low-, mid-, and high-level features using multimodal data [@celma_foafing_2006], or train neural networks to automatically learn features from audio [@zhang2016br;@alvarez2023ri;@agarwal2021an]. Through these approaches, MER researchers attempt to shatter a so-called *glass ceiling* [@downie_music_2008] by establishing central predictors for emotions. To date, however, no study has systematically compared results of these diverse approaches.

<!-- added more recent references to make the neural network mention more up to date, TE 19/3/2025 -->
...
```{=html}
<!-- Question to Cameron: Could you add some of the studies included in the meta-analysis in the references to those studies that do not share datasets to make this more relevant. I just copied two references that I know are examples of datasets that require extra steps, but we have studies where we just don't even know whether the data is available, right? 

CA: I've added some studies now, though these were excluded from the final meta-analysis. I also have a few examples of those which were included in the final results based on scrolling through the articles. Many are psychology papers, where it seems less common to share data publicly, but those also tend to have more case-specific goals rather than comparing results to existing benchmarks, so I didn't cite them. @hizlisoy2021mu proposes a new turkish music database, but I wasn't able to find the data from the article, so I added that. @zhang2017fe uses a dataset but doesn't seem to provide information on how to access it.
TE: great, well done, job done. -->
```
...
<!-- This last section needs to be expanded. Or text moved from above to here? -->
