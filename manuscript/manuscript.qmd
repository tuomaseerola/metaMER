...
In the early 2000s, several research groups conducted studies using regression [@friberg_automatic_2002; @liu_automatic_2003] and classification [@lu_automatic_2005; @feng_popular_2003; @mandel_support_2006] techniques to predict emotion in music audio or MIDI. Citing "MIR researchers' growing interest in classifying music by moods" [@downie_music_2008, p. 1], the Music Information Retrieval EXchange (MIREX) introduced Audio Mood Classification (AMC) to their rotation of tasks in 2007. In the first year, nine systems classified mood labels in a common data set, reaching 52.65% accuracy (SD = 11.19%). These events, along with growing interest in the burgeoning field of affective computing, would lead to an explosion of interest in MER research.

<!--Paragraph on MER as regression task-->

In the tenth annual AMC task, the highest performing model reached 69.83% accuracy [@park2017representation]. In parallel, other researchers began assessing MER as a regression task. The first study to popularize this approach predicted valence (i.e., the negative---positive emotional quality) and arousal (i.e., the calm---exciting quality) in 195 Chinese pop songs [@yang2008] using audio-extracted features. The regression model predicted arousal with 62% accuracy, but only achieved 28% accuracy for valence. This difference in prediction accuracy between dimensions has reappeared in several subsequent studies [e.g., @bai2016dimensional; @coutinho2013psychoacoustic], with some research suggesting this challenge reflects fewer well-established predictors and more individual differences for valence than arousal [@yang2007music; @eerola2011c].

<!--paragraph on models used across regession & classification tasks-->

Across regression and classification paradigms, a wide range of models have been employed, ranging from linear regression models [@saizclar2022pr; @griffiths2021am; @yang2008] to deep neural networks [@orjesek2022en; @hizlisoy2021mu]. In classification tasks, early work has commonly employed Gaussian mixture models [@lu_automatic_2005; @liu_automatic_2003] and support vector machines [@mandel2007labrosa; @lin2009eeg; @tzanetakis2007marsyas], whereas convolutional, recurrent, and fully-connected neural networks have become increasingly popular in recent years [@grekow2021music; @song2018audio; @coutinho2017sh]. In regression tasks, a wide range of algorithms have been tested, including partial least squares [@gingras2014be; @wang2021ac], support vector machines [@grekow2018au; @hu2017cr; @agarwal2021an], random forests [@beveridge2018po; @xu2021us], and convolutional neural networks [@orjesek2022en].

## Dataset size and scope

MER tasks commonly apply regression and classification techniques to diverse datasets with features drawn from both music (e.g., audio, MIDI, metadata) and participants (e.g., demographic information, survey responses, physiological signals, etc.). To facilitate model comparison, several databases have been shared publicly, including _MediaEval_ [@soleymani2013], _DEAM_ [@aljanaki2017developing], and _AMG1608_ [@chen2015amg1608]. These datasets represent Western pop music, are moderate in terms of the size (containing from 744 to 1802 music excerpts) and have been manually annotated by a relatively large number of participants (either by experts, students, or crowdsourced workers). Many datasets include audio features analyzed with software toolboxes for audio analysis such as _OpenSMILE_ [@eyben2010opensmile] and _MIRToolbox_ [@lartillot2007matlab] -- enabling models that predict ratings using tens, or even hundreds, of audio features. 

Large datasets are needed to generalize predictions across diverse samples, especially for deep learning and complex machine-learning models that require extensive pre-training. However, the dataset sizes in MER tend to be modest in comparison, as direct annotation of music examples is resource-intensive. In contrast, datasets in the visual domain are often significantly larger (e.g., _EmoSet_ with 118,102 images and _AffectNet_ with 450,000, see @yang2023emoset;@mollahosseini2017affectnet). Some efforts have been made to scale up MER datasets by inferring emotions from tags (_MTG-Jamendo_ with 18,486 excerpts @bogdanov2019mtg and _Music4all_ with 109,269 excerpts @santana2020), but these have not found their way into standard emotion prediction or classification tasks yet.

Although training MER systems with a wide range of stimuli and features can be useful for generalizable and accurate modeling, small datasets can be more appropriate in explorations that require precise control of stimuli or analyzed features to understand a model's performance. For example,  @griffiths2021am evaluated a MER model's performance when predicting 20 tracks different genres, validating the model on another 40 tracks. @saizclar2022pr evaluated how onset curves (changes in the energy of a note starting from its onset) can be incorporated into a predictive framework using 40 short (8-12s) piano excerpts. Smaller datasets are also useful in studies attempting to explain how stimulus qualities such as amplitude normalization [@gingras2014be] or different performers' interpretations [@battcock2021in] affect ratings of valence and arousal. Although predictive modeling is not always an explicit goal of these studies, MER researchers have been able to adapt them in a MER framework [e.g., @chowdhury2021perceived]. 
...
- How do modeling techniques, and the number of features used affect performance? Which combinations perform best?

-->
```

Prediction accuracy in MER tasks has improved as datasets and models have become more sophisticated; The regression models for arousal/valence was 58%/28% in 2008 [@yang2008], 70%/26% in 2010 [@huq2010automated], to 67%/46% in 2021 [@yang2021an]. The classification rates have similarly increased from 53% [@downie_music_2008] to 70% [@park2017representation] to 83% [@sarkar2020recognition] in the same period. Comparing these past efforts, however, is challenging due to inconsistencies in metrics, modelling architectures, datasets, and model evaluation criteria. While we assume that overall accuracy has improved significantly over the past decade, predicting valence remains more challenging than predicting arousal.

Researchers have sought to enhance emotion prediction by identifying more relevant feature sets [@panda2020audio; @chowdhury2021perceived], integrating low-, mid-, and high-level features through multimodal data [@celma_foafing_2006], and leveraging neural networks to learn features directly from audio [@zhang2016br; @alvarez2023ri; @agarwal2021an]. These approaches aim to overcome what has been described as a _glass ceiling_ in MER research [@downie_music_2008] -- a limitation sometimes referred to as the _semantic gap_ [@wiggins_semantic_2009;@celma_foafing_2006]. However, this prediction ceiling may be better understood as an inherent _measurement error_ arising from annotations, feature representations, and model limitations. While isolating the sources of these errors remains infeasible at this stage, comparing success rates across modeling techniques, feature set sizes, and other meaningful factors offers a step toward addressing this challenge. To date, however, no study has systematically compared the results of the diverse approaches employed in MER research. 
...
To capture key details of each study, we added extra fields to BibTeX entries for each study. Fields included information about the genre/type of stimuli employed, along with their duration and number; the number of analyzed features; and the model type -- Neural Nets (NN), Support Vector Machines (SVM),[^2] Linear Methods (LM), Tree-based Methods (TM), KS, Add. and KNN Methods (KM) -- validation procedure and output measures. Additionally, we included study results using executable *R* code containing custom functions for meta-analysis. For complete details about our encoding procedure, see `studies/extraction_details.qmd` .

[^2]: @chen2017co was the only study of those included to use Gaussian Mixture Models. We decided to group this with Support Vector Machines as they have been reported to perform similarly on mid-sized data sets [@mashao2003co].
...
<!-- numbers in the text updated up to here, CA 20/2/2025 -->
<!-- DISCUSS: we don't report journal type of explain/predict differences, although we do some of these in arousal, TE 19/3/2025 -->
...
The analysis of the subdivision of studies shows that there is no significant differences between the studies using different number of features (*Q*(2) = 5.20, *p* = .074) despite the differing means (*r* = 0.885 for studies with less than 30 features, *r* = 0.735 for 30 to 300 features, and *r* = 0.804 for studies utilising over 300 features). The differences in the techniques, however, does show statistically significant variance between subgroups (*Q*(3) = 10.83, *p* = .0127). The Neural Nets (NN) achieve poor prediction of arousal (*r* = 0.660) in comparison to other techniques. The caveat of this subgroup analysis is the small number of observations for four techniques. When other sub-groupings were explored, such as the type of journal (Engineering with 13 studies vs Psychology with 9 studies), no significant differences were observed (*Q*(1) = 0.94, *p* = .333).
...
The number of features (as classified into under 30, between 30 and 300, and above 300 features; see Table 3 for actual ranges) analyzed does not significantly impact results (*Q*(2) = 3.91, *p* = 0.1419), despite $MCC$s showing average differences according to the number of features (\< 30 F: $MCC$ = 0.9293; 30-300 F: $MCC$ = 0.8458; \>300 F: $MCC$ = 0.7754). Similarly, the class of model did not differ significantly (*Q*(3) = 4.22, *p* = 0.239) despite neural networks attaining highest average performance ($MCC$ = 0.9307), followed by SVMs ($MCC$ = 0.870) and tree-based methods ($MCC$ = 0.854), then linear methods ($MCC$ = 0.728). Finally, neither single- vs. multigenre studies (*Q*(1) = 0.12, *p* = 0.7324), nor binary vs. multi-class models (*Q*(1) = 0.03, *p* = 0.8694) differed significantly. Unlike regression, no classification studies were published in engineering journals.

### Model success across concepts, model types and feature counts

<!-- Changed the header slightly, TE 19/3/2025-->

To assess how the use of different model types affected performance, we prepared heatmap visualizations depicting differences in success across feature *n* categories. We collapsed SVM and Tree-Based categories due to their low representation in the model summary. Figure 5 summarizes differences in success (a) across categories, as well as (b) the algorithms in each model class. Overall, studies using smaller feature sets tend to perform best, whereas the best model type largely depends on the nature of the prediction task.
...
Research on Music Emotion Recognition has steadily increased in popularity since the early 2000s, with technological advancements facilitating sharing of data sets, analysis tools, and music stimuli. Public forums like the Music Information Retrieval Exchange (MIREX) have facilitated collaborations between computer scientists, musicologists, and psychologists alike—spurring improvements in performance. Despite increasing complexity in the types of models and data sets used, no extant study has evaluated the relative merits of the tools, stimuli, and feature sets employed in MER studies. The present study offers the first meta-analysis of music emotion recognition.

<!-- Concise summary of what we did and found -->

We initially identified 96 studies involving MER models, but narrowed our selection to 34 after filtering studies to ensure consistent quality, reporting standards, and a dedicated focus on music. From these studies, we encoded accuracy scores for 290 models, with 204 related to regression and 86 to classification. Comparing the most accurate model in each study revealed reasonably accurate prediction of valence (*r* = 0.67 \[0.56, 0.76\]), and arousal (*r* = 0.81 \[0.74, 0.86\]) in regression studies. For both affective dimensions of the circumplex model, linear methods (valence *r* = 0.78, arousal *r* = 0.88) and tree-based models (valence *r* = 0.75, arousal *r* = 0.81) outperformed neural networks (valence = 0.47, arousal = 0.66) and support vector machines (SVMs) (valence *r* = 0.58, arousal *r* = 0.79). In contrast, neural networks performed most accurately in classification studies ($MCC$ = 0.93), followed by SVMs ($MCC$ = 0.87) and tree-based models ($MCC$ = 0.85). Despite the high overall success of the research in this topic, the models exhibited several differences relating to (i) the scale and quantity of data sets employed, (ii) feature extraction methods, (iii) the number and types of features and reduction methods used, (iv) the actual modeling architecture, and (v) how model outcomes are cross-validated.

## Improvements to predictive accuracy

The results of the meta-analysis offer several important insights. First, compared to a 2013 report by @barthet2013, the glass ceiling for valence has risen from *r* = 0.51 to 0.67.[^3] Conversely, predictive accuracy for arousal has shown no improvement (reported already in 2013 at *r* = 0.83). For classification, studies reached variable classification rates from 0.497 using *F~1~* score [@sanden2011empirical] to 79% average precision [@trohidis2008multi] in the past, whereas now the accuracy is generally notably better, here $MCC$ = 0.87, albeit the differences in metrics.

[^3]: Values reported as $R^2$ in original study.
...
Finally, we note that the relevance of the emotion frameworks used in MER is not always explicitly discussed. The majority of studies rely either on valence and arousal or some combination of basic emotion categories. However, these frameworks may have limited usefulness for some practical applications that aim to capture experiences with music that are more aligned with models of music-induced emotions such as GEMS by @zentner2008emotions or AESTHEMOS [@schindler2017me], or what emotions music are shown to be able to express [@eerola2025what], or what are assumed to be worthwhile clusters of concepts (moods, emotions, tags) from crowdsourced, non-theory driven data [@saari_et_al_2015].
