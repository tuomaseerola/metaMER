...
Now we can convert our list into a data frame with the target bibtex fields. For the last field `FINAL_NOTES`, we will apply a different regex pattern which matches all characters following the field name `(?<=FINAL_NOTES).*` .
...
included_studies |> dplyr::tibble() #|> View()
```
# Track studies to discuss
```{r}
included_studies[str_detect(included_studies$final_notes, 'DISCUSS|Discuss|discuss'),] 
```
...
resolution_comments <- c(
  'Report total # of features 8904',
  'Report available statistics (new nomenclature solves issue)',
  'Report as many features are there are in MEM n = 260',
  'Report best model results (Rprop)',
  'Report available statistics (new nomenclature solves issue)',
  'Report on proposed model',
  'Report 50 PCA features'
)

included_studies[str_detect(included_studies$final_notes, 'DISCUSS|Discuss|discuss'),]$final_notes <- resolution_comments

```

# Track Reformatted Extractions

```{r}
included_studies[which(stringr::str_detect(included_studies$model_rate_emotion_values,
                                     'bind_field')),] -> reencoded

message('Completed re-encodings for ', nrow(reencoded), ' studies')
...
```{r}
check_missing_fields <- function(n_empty_fields = 1) 
{
...
get_nested_studies = function(df)
{ # loop across rows
