...
```{=html}
<!-- 
- Types of classification algorithms employed
- Popularity of SVMs (for both classification and regression)
- Algorithms used for personalized and nonpersonalized MER models?
- Success with regression varieties
- Introduction of neural networks & deep learning paradigms
-->
```

<!--summary of feature sets and quantity-->
```{=html}
<!-- 
- Wide set of features explored
- Summary of categories commonly used/discussed (Panda)
- Audio, midi, lyric features
- Low-, mid-, and high-level features
- Emphasize focus on audio-based featuers
- Small, medium, and large data sets 
- Could emphasize that types of features differ based on type of study
(psychology, engineering) due to different goals
-->
```
...
### Model success across concepts

To assess how the use of different model types affected performance, we prepared heatmap visualizations depicting differences in success across feature *n* categories. We collapsed SVM and Tree-Based categories due to their low representation in the model summary. Figure 5 summarizes differences in success across categories (a), as well as the algorithms in each model class (b). Oveall, studies using smaller feature sets tend to perform best, whereas the best model type largely depends on the nature of the prediction task.

![Heatmaps summarizing models. 5(a) depicts overall success across tasks according to model type and feature *n* summaries. Text labels summarize the number of models belonging to each subgroup, whereas the numbers in parentheses summarize the median number of stimuli used. 5(b) summarizes the algorithms pertaining to each class, colour-coded by frequency. Grey rectangles are visual aids aligning with *x*-axis groupings in 5(a) (white: LM; light grey: NN; dark grey: other).](../manuscript/model-summary-fig.png "Heatmap depicting success across tasks according to Feature N category and model class. Labels indicate the number of studies and the median number of stimuli is shown in parentheses."){width="668"}
