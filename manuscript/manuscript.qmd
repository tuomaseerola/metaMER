...
Although training MER systems with a wide range of stimuli and features can be useful for generalizable and accurate modelling, smaller datasets can be more appropriate in studies requiring precise control over stimulus parameters. For example, @griffiths2021am tested multi-genre MER using multiple linear regression to predict emotion in 20 tracks from different genres (validating the model using a separate set of 40 tracks). @saizclar2022pr evaluated how 22 variables derived from an onset curves (changes in the energy of a note starting from its onset) can be used to predict valence and activation in 40 short (8-12s) piano excerpts. Smaller datasets are also useful in studies attempting to explain how stimulus qualities such as amplitude normalization [@gingras2014be] or different performers' interpretations [@battcock2021in] affect ratings of valence and arousal. Although predictive modelling is not always an explicit goal of these studies, they can be adapted to fit into a MER framework. For example, @chowdhury2021perceived adopted data from a previous psychological study of piano performances to compare sets of features analyzed from audio files, musical scores, or deep neural network feature extractors.
...
To capture key details of each study, we added extra fields to BibTeX entries for each study. Fields included information about the genre/type of stimuli employed, along with their duration and number; the number of analyzed features; and the model type -- Neural Nets (NN), Support Vector Machines (SVM),[^1] Linear Methods (LM), Tree-based Methods (TM), KS, Add. and KNN Methods (KM) -- validation procedure and output measures. Additionally, we included study results using executable *R* code containing custom functions for meta-analysis. For complete details about our encoding procedure, see `studies/extraction_details.qmd` .
